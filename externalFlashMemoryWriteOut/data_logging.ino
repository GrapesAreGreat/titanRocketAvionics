...
void FlashLogger::log_string(char *s, const size_t s_len) {
  if (this->currentInternalBufferLength + s_len >= MAX_INTERNAL_BUFFER_LEN) {
    ...
    this->flash.loadRandProgData(currentPage, s, bytes_can_write);
    ...
    this->flash.loadRandProgData(currentPage, s_next, s_next_length);

  } else {
    this->flash.loadRandProgData(currentPage, s, s_len);
  }
}

void FlashLogger::flush() {
  this->flash.programExecute(currentPage);
}
